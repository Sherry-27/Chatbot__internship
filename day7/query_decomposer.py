...
def synthesise_answers(original_question, sub_questions, sub_answers):
    ...
    synthesis_prompt = f'''You have been given a complex question and a set of focused answers to its sub-questions.
Synthesise all the sub-answers into one comprehensive, well-structured final answer to the original question.
Use only the information in the sub-answers. Be concise.
...
def decompose_and_answer(question, retriever, rerank_fn, generate_fn):
    ...
    if len(sub_questions) == 1:
        ...
        print('\nSynthesising sub-answers...')
        final_answer = synthesise_answers(question, sub_questions, sub_answers)
    return {'sub_questions': sub_questions, 'sub_answers': sub_answers, 'final_answer': final_answer}
