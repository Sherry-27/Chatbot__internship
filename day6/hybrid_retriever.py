...
class HybridRetriever:
    ...
    def index(self, doc_paths, collection_name='nexuschat_hybrid'):
        ...
        self.collection.add(ids=all_ids, embeddings=all_embeddings, documents=all_chunks, metadatas=[{'source': s} for s in all_sources])
    ...
    def search(self, query, top_k=3, fetch_k=10):
        ...
        raw = self.collection.query(query_embeddings=[query_vec], n_results=fetch_k, include=['documents', 'metadatas', 'distances'])
