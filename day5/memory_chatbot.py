...
Rewrite the following follow-up question as a standalone question that does not depend on the conversation history. If the question is already standalone, return it unchanged. Output ONLY the rewritten question, nothing else.
...
def generate_with_memory(question, retrieved_chunks, chat_history):
    ...
    if chat_history:
        ...
        for turn in chat_history[-HISTORY_TURNS:]:
            ...
            history_lines.append(f"Assistant: {turn['answer']}")
        history_block = '\n'.join(history_lines)
        base_prompt = base_prompt.replace('Question:', f'Conversation so far:\n{history_block}\n\nQuestion:')
    model = genai.GenerativeModel(GENERATION_MODEL)
